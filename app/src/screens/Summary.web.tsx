...
export default function Summary() {
  ...
  // Mirrors the one-level-down drilling done by the calendar's own day/month cells (year -> month
  // -> week -> day) -- no entry for "year" since that's already the top level.
  const ZOOM_OUT: Partial<Record<SumPeriod, SumPeriod>> = { day: "week", week: "month", month: "year" };
  const zoomOut = () => {
    const next = ZOOM_OUT[sumPeriod];
    if (next) setSumPeriod(next);
  };
        ...
        <View style={[styles.row, isMobile && styles.rowStacked]} testID="summary-calendar-row">
          <View style={[styles.card, styles.calendarCard, rowFlex(7), shadow.card]}>
            <View style={styles.calendarHeaderRow}>
              ...
              <View style={styles.calCaptionRow}>
                {sumPeriod !== "day" && <Text style={styles.calCaption}>darker = more spent</Text>}
                {ZOOM_OUT[sumPeriod] && (
                  <Pressable onPress={zoomOut} style={styles.toggleButton} testID="cal-zoom-out">
                    <Text style={styles.toggleButtonText}>Zoom out</Text>
                  </Pressable>
                )}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  calCaptionRow: { flexDirection: "row", alignItems: "center", gap: spacing.sm },
